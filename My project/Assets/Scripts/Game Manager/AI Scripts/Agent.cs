...
using Game_Manager.AI_Scripts.Navigation;
...
namespace Game_Manager.AI_Scripts
{
    ...
    public abstract class Agent : MessageComponent
    {
        ...
        /// </summary>
        public List<Movement> Moves { get; private set; } = new List<Movement>();
        ...
        /// </summary>
        public Vector3? Destination => Path.Count > 0 ? Path[Path.Count - 1] : (Vector3?)null;
        ...
        {
            if (Moves.Exists(m => m.Behaviour == behaviour && m.Transform == tr) || Steering.IsMoveComplete(behaviour, new Vector2(transform.position.x, transform.position.z), new Vector2(tr.position.x, tr.position.z)))
            {
                return;
            }
            
            RemoveMove(tr);
            Moves.Add(new Movement(behaviour, tr));
        }
        ...
        {
            if (Moves.Exists(m => m.Behaviour == behaviour && m.Transform == null && m.Position == pos) || Steering.IsMoveComplete(behaviour, new Vector2(transform.position.x, transform.position.z), pos))
            {
                return;
            }
            
            RemoveMove(pos);
            Moves.Add(new Movement(behaviour, pos));
        }
        ...
        protected void CalculateMoveVelocity(float deltaTime)
        {
            ...
            if (Path.Count > 0)
            {
                ...
                while (Path.Count > 0 && Vector2.Distance(position, new Vector2(Path[0].x, Path[0].z)) <= Manager.SeekAcceptableDistance)
                {
                    Path.RemoveAt(0);
                }
            ...
            MoveVelocity = new Vector2((float) x, (float) y);
        }

...
namespace Game_Manager.AI_Scripts
{
    ...
    public class Manager : MonoBehaviour
    {
        ...
        /// The global messages.
        /// </summary>
        private List<string> _globalMessages = new List<string>();
        ...
        /// </summary>
        private readonly List<Agent> _fixedUpdateAgents = new List<Agent>();
        ...
        private static void AgentGizmos(Agent agent)
        {
            ...
            else
            {
                ...
                foreach (Agent.Movement movement in agent.Moves)
                {
                    ...
                    GL.Vertex(position);
                    GL.Vertex(new Vector3(movement.Position.x, position.y, movement.Position.y));
                }
            }
        }
            ...
            if (loadLookupTable)
            {
                if (lookupTable != null)
                {
                    ...
                    foreach (NavigationLookup lookup in _navigationTable)
                    {
                        ...
                        {
                            _connections.Add(new Connection(lookup.current, lookup.next));
                        }
                    }
                }
                else
            ...
            if (!loadLookupTable)
            {
                ...
                foreach (Node node in FindObjectsOfType<Node>())
                {
                    ...
                    foreach (Vector3 v in _nodes)
                    {
                        ...
                        _connections.Add(new Connection(p, v));
                    }
                ...
                for (int i = 0; i < _nodes.Count; i++)
                {
                    ...
                    for (int j = 0; j < _nodes.Count; j++)
                    {
                        ...
                        for (int k = 0; k < path.Count - 1; k++)
                        {
                            ...
                            NavigationLookup lookup = new NavigationLookup(path[k], _nodes[j], path[k + 1]);
                            table.Add(lookup);
                        }
                    }
                }

...
using System.IO;
using System.Linq;
using Game_Manager.AI_Scripts.Navigation.Generators;
...
namespace Game_Manager.AI_Scripts.Navigation.Nodes
{
    ...
    public class NodeArea : NodeBase
    {
        ...
        public void Generate()
        {
            ...
            for (int x = 0; x < RangeX; x++)
            {
                for (int z = 0; z < RangeZ; z++)
                {
                    ...
                    _data[x, z] = Physics.Raycast(new Vector3(pos.x, floorCeiling.y, pos.y), Vector3.down, out RaycastHit hit, floorCeiling.y - floorCeiling.x, Manager.GroundLayers | Manager.ObstacleLayers) && (Manager.GroundLayers.value & (1 << hit.transform.gameObject.layer)) > 0
                        ? Open
                        : Closed;
                }
            }
            ...
            if (generator != null)
            {
                ...
                for (int x = 0; x < _nodes.Count; x++)
                {
                    for (int z = 0; z < _nodes.Count; z++)
                    {
                        ...
                        Manager.Connections.Add(new Connection(_nodes[x], _nodes[z]));
                    }
                }
            }
        ...
        public void AddNode(int x, int z)
        {
            ...
            if (Physics.Raycast(new Vector3(pos.x, floorCeiling.y, pos.y), Vector3.down, out RaycastHit hit, floorCeiling.y - floorCeiling.x, Manager.GroundLayers))
            {
                y = hit.point.y;
            }
